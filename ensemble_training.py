...
def _scalar_loss_numpy(loss_function_choice, y_true, y_pred, huber_delta):
    """Compute a scalar loss using NumPy only, to avoid Keras name_scope issues."""
    y_true = np.asarray(y_true, dtype=np.float32).ravel()
    y_pred = np.asarray(y_pred, dtype=np.float32).ravel()
    err = y_true - y_pred

    if loss_function_choice == "Mean Squared Error":
...
def train_ensemble_model(
    ...
    # Convenience flags for pure DNN / pure base
    use_pure_dnn = alpha >= 1.0
    use_pure_base = alpha <= 0.0
    ...
    for epoch in range(epochs):
        ...
        y_pred_base_train = np.asarray(y_pred_base_train).ravel()
        y_pred_base_val = base_model_instance.predict(X_val_scaled)
        y_pred_base_val = np.asarray(y_pred_base_val).ravel()

        if use_pure_dnn:
            y_pred_ens_train = y_pred_dnn_train
            y_pred_ens_val = y_pred_dnn_val
        elif use_pure_base:
            y_pred_ens_train = y_pred_base_train
            y_pred_ens_val = y_pred_base_val
        else:
            y_pred_ens_train = (
        ...
        # Losses – NumPy only, to avoid Keras name_scope bug
        dnn_train_loss = _scalar_loss_numpy(
            loss_function_choice, y_train, y_pred_dnn_train, huber_delta
        )
        dnn_val_loss = _scalar_loss_numpy(
            loss_function_choice, y_val, y_pred_dnn_val, huber_delta
        )
        ens_train_loss = _scalar_loss_numpy(
            loss_function_choice, y_train, y_pred_ens_train, huber_delta
        )
        ens_val_loss = _scalar_loss_numpy(
            loss_function_choice, y_val, y_pred_ens_val, huber_delta
        )
    ...
    y_pred_dnn_val_final = dnn_model.predict(X_val_scaled, verbose=0).flatten()
    y_pred_base_val_final = base_model_instance.predict(X_val_scaled)
    y_pred_base_val_final = np.asarray(y_pred_base_val_final).ravel()

    if use_pure_dnn:
        y_pred_ensemble = y_pred_dnn_val_final
    elif use_pure_base:
        y_pred_ensemble = y_pred_base_val_final
    else:
        y_pred_ensemble = (
            alpha * y_pred_dnn_val_final + (1 - alpha) * y_pred_base_val_final
        )
...
def _summarize_folds(
    ...
    for k, (_, va) in enumerate(folds):
        parts.append(pd.DataFrame({"fold": k, "country": groups[va]}))
    s = pd.concat(parts)
    table = (
        s.groupby(["fold", "country"])
        .size()
        .rename("rows")
        .reset_index()
...
def cross_validate_ensemble(
    ...
    groups=None,
    sample_weights=None,
    # NEW: group balancing options
    balance_groups=False,  # NEW
    stratify_bins=False,  # NEW
    # training params
    epochs=200,
    ...
    if cv_type == "kfold":
        ...
        if balance_groups:
            # Use balanced group splitter (keeps entire groups)
            y_bins = _make_bins(y) if stratify_bins else None
            ...
            # vanilla GroupKFold (note: not stratified)
...
def show_ensemble_training_tab(df):
    ...
    if use_cv:
        ...
        if cv_type == "groupkfold":
            ...
            # NEW toggles for balanced group CV
