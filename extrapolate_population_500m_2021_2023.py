...
FILE = BASE_DIR / 'gee_all_vars_added' / 'all_82_population_500m_original_ref_gaul_actual.csv'
...
def main() -> None:
    ...
    # Extrapolation can produce small negatives — clip all population metrics to 0
    for col in METRIC_COLS:
        if col in df.columns:
            df[col] = df[col].clip(lower=0)

    if 'system:index' in df.columns or '.geo' in df.columns:
        ordered = [c for c in ['system:index', 'Country', 'Max Population', 'Mean Population',
                                'Median Population', 'Min Population', 'Region',
                                'Std Dev Population', 'Total Population', 'Year', '.geo']
                   if c in df.columns]
